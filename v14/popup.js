...
async function loadState() {
  try {
    ...
    bestScore = typeof state.bestScore === 'number' ? state.bestScore : 0;
    bestCombo = state.bestCombo && typeof state.bestCombo === 'object' ? state.bestCombo : null;
  ...
  chrome.scripting.executeScript({
    ...
    func: function(delayMs) {
      ...
      const uavDroneConstraints = {
        ...
        "N.A. Website Product Displays/Info": { min: 0, max: 5000, initialSteps: [0, 50, 100, 250, 500, 750, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000] },
        "E-A Website Product Displays/Info": { min: 0, max: 5000, initialSteps: [0, 50, 100, 250, 500, 750, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000] },
        "A-P Website Product Displays/Info": { min: 0, max: 5000, initialSteps: [0, 50, 100, 250, 500, 750, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000] },
        "L.A. Website Product Displays/Info": { min: 0, max: 5000, initialSteps: [0, 50, 100, 250, 500, 750, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000] },
        "N.A. Search Engine Advertising": { min: 0, max: 15000, initialSteps: [0, 50, 100, 500, 1000, 1500, 2000, 2500, 3000, 4000, 5000, 6000, 7500, 9000, 10500, 12000, 13500, 15000] },
        "E-A Search Engine Advertising": { min: 0, max: 15000, initialSteps: [0, 50, 100, 500, 1000, 1500, 2000, 2500, 3000, 4000, 5000, 6000, 7500, 9000, 10500, 12000, 13500, 15000] },
        "A-P Search Engine Advertising": { min: 0, max: 15000, initialSteps: [0, 50, 100, 500, 1000, 1500, 2000, 2500, 3000, 4000, 5000, 6000, 7500, 9000, 10500, 12000, 13500, 15000] },
        "L.A. Search Engine Advertising": { min: 0, max: 15000, initialSteps: [0, 50, 100, 500, 1000, 1500, 2000, 2500, 3000, 4000, 5000, 6000, 7500, 9000, 10500, 12000, 13500, 15000] },
        "N.A. Retailer Recruitment/Support Budget": { min: 0, max: 10000, initialSteps: [0, 100, 500, 1000, 1500, 2000, 2500, 3000, 4000, 5000, 6000, 7500, 9000, 10000] },
        "E-A Retailer Recruitment/Support Budget": { min: 0, max: 10000, initialSteps: [0, 100, 500, 1000, 1500, 2000, 2500, 3000, 4000, 5000, 6000, 7500, 9000, 10000] },
        "A-P Retailer Recruitment/Support Budget": { min: 0, max: 10000, initialSteps: [0, 100, 500, 1000, 1500, 2000, 2500, 3000, 4000, 5000, 6000, 7500, 9000, 10000] },
        "L.A. Retailer Recruitment/Support Budget": { min: 0, max: 10000, initialSteps: [0, 100, 500, 1000, 1500, 2000, 2500, 3000, 4000, 5000, 6000, 7500, 9000, 10000] }
      };
      ...
      async function optimizeInput(input, globalState = { bestCombo: {}, bestEPS: 0 }) {
        ...
        const originalValue = bestValue;
        let incrementResult = null;
        let decrementResult = null;
    
        // Try incrementing by 100
        const incrementedValue = Math.min(bestValue + 100, inputConstraints.max);
        if (incrementedValue !== bestValue) {
            setInputValue(input, incrementedValue);
            await clickCalculate();
            await new Promise(r => setTimeout(r, 2));
    
            const incrementedMetrics = getMetrics();
            
            if (incrementedMetrics.eps > bestEPS) {
                incrementResult = {
                    value: incrementedValue,
                    eps: incrementedMetrics.eps,
                    metrics: incrementedMetrics
                };
            }
        }
    
        // Try decrementing by 100
        const decrementedValue = Math.max(bestValue - 100, inputConstraints.min);
        if (decrementedValue !== bestValue) {
            setInputValue(input, decrementedValue);
            await clickCalculate();
            await new Promise(r => setTimeout(r, 2));
    
            const decrementedMetrics = getMetrics();
            
            if (decrementedMetrics.eps > bestEPS) {
                decrementResult = {
                    value: decrementedValue,
                    eps: decrementedMetrics.eps,
                    metrics: decrementedMetrics
                };
            }
        }
    
        // Determine best result from increment/decrement
        if (incrementResult && (!decrementResult || incrementResult.eps > decrementResult.eps)) {
            bestValue = incrementResult.value;
            bestEPS = incrementResult.eps;
            
            chrome.runtime.sendMessage({
                type: 'newBestScore',
                score: bestEPS,
                metrics: incrementResult.metrics,
                combo: captureCurrentValues(),
                timestamp: new Date().toLocaleTimeString()
            });
        } else if (decrementResult) {
            bestValue = decrementResult.value;
            bestEPS = decrementResult.eps;
            
            chrome.runtime.sendMessage({
                type: 'newBestScore',
                score: bestEPS,
                metrics: decrementResult.metrics,
                combo: captureCurrentValues(),
                timestamp: new Date().toLocaleTimeString()
            });
        } else {
        ...
        // More granular search for other inputs
            ...
            const increments = [100, 50, 25, 10, 5, 4, 3, 2, 1];
            let localBestEPS = bestEPS;
            let localBestValue = bestValue;
      ...
      async function optimize() {
        ...
        // Safely wrap chrome.runtime.sendMessage to handle potential errors
        ...
        const updateGlobalState = (metrics, combo) => {
          ...
          // Update window-level best values
          window.bestCombo = { ...combo };
          window.bestMetrics = { ...metrics };
        };
      
        // Ensure window-level best values are initialized
        window.bestCombo = {};
        window.bestMetrics = null;
      
        while (isRunning) {
          try {
            // Get current metrics at the start of each iteration
            const currentMetrics = getMetrics();
      
            // Check if current metrics are better than global best
            if (currentMetrics.eps > globalState.bestEPS) {
              const currentCombo = captureCurrentValues();
              updateGlobalState(currentMetrics, currentCombo);
            }
      
            // Apply best global values if available
            ...
            // Define strategies for different input types
            const strategies = [
...
function stopOptimizer(tab) {
  chrome.scripting.executeScript({
    ...
    func: () => {
      ...
      // Apply the best global values if they exist
      if (window.bestMetrics && Object.keys(window.bestMetrics).length > 0) {
        const inputs = document.querySelectorAll('input.input-field');

        // Apply best input values
        inputs.forEach(input => {
          const label = input.getAttribute("title") || input.getAttribute("aria-label");
          const bestCombo = window.bestCombo || {};
          
          if (bestCombo[label] !== undefined) {
            const value = bestCombo[label].value !== undefined 
              ? bestCombo[label].value 
              : bestCombo[label];
        ...
        // Click calculate to update scores
        const calcBtn = Array.from(document.getElementsByTagName('button')).find(b => 
          b.textContent.toLowerCase().includes('calculate') || 
          b.textContent.toLowerCase().includes('update scores')
        );
  ...
  // Update local state to reflect the best values
  isRunning = false;
  saveState();
}
