  ...
  chrome.scripting.executeScript({
    ...
    func: function(delayMs) {
      ...
      function calculateCompositeScore(metrics) {
        if (!metrics || !metrics.eps || isNaN(metrics.eps)) return null;
        const eps = parseFloat(metrics.eps);
...
const acCameraConstraints = {
  // Wholesale Prices - Refined increments for precise price optimization
  ...
  // Retailer Support Budget - Enhanced steps for optimal budget allocation
  ...
  // Advertising Budget - More granular steps for precise marketing optimization
  ...
  // Website Product Displays - Refined steps for better online presence optimization
...
const uavDroneConstraints = {
  // Retail Prices - More granular increments for precise optimization
  ...
  // Website Product Displays - More granular steps for fine-tuning
  ...
  // Search Engine Advertising - More precise steps for better optimization
  ...
  // Retailer Recruitment/Support - Refined steps for better budget allocation
...
function detectProduct() {
  ...
  for (const input of inputs) {
    ...
    if (!label) continue;
    if (label.includes('Wholesale Price')) {
...
function setInputValue(input, value) {
  ...
  if (product === "acCamera") {
    // Direct match for AC Camera
    constraints = acCameraConstraints[normalizedLabel];
  } else if (product === "uavDrone") {
    // Try exact match first
    constraints = uavDroneConstraints[normalizedLabel];
    
    // If no exact match, try matching without "Average" for retail prices
    if (!constraints && normalizedLabel.includes('Retail Price')) {
      const region = normalizedLabel.split(' ')[0]; 
      const alternativeKey = `${region} Average Retail Price`;
      constraints = uavDroneConstraints[alternativeKey];
    }
  }

  if (!constraints) {
    console.log(`No constraints found for label: ${normalizedLabel} (Product: ${product})`);
    return;
  }
