...
async function handleNewScore(currentScore, currentCombo, metrics) {
  if (currentScore && currentScore >= bestScore) { // Changed > to >= to include equal scores
      if (currentScore > bestScore) {
          // Only update if strictly better
          bestScore = currentScore;
          globalBestCombo = currentCombo;
          globalHistory = []; // Clear history when new best is found
      }
      
      // Add to history only if it equals the best score AND has better secondary metrics
      if (currentScore === bestScore && shouldAddToHistory(metrics)) {
          globalHistory.push({
              score: currentScore,
              combo: currentCombo,
              metrics: metrics,
